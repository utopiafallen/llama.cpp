...
static void add_tensor(ggml_tensor * tensor, const std::shared_ptr<rpc_command_queue> & cmd_queue_ref, std::vector<rpc_tensor> & tensors, std::unordered_set<ggml_tensor*> & visited) {
    if (tensor == nullptr) {
        return;
    }
    ...
    for (int i = 0; i < GGML_MAX_SRC; i++) {
        add_tensor(tensor->src[i], cmd_queue_ref, tensors, visited);
    }
    add_tensor(tensor->view_src, cmd_queue_ref, tensors, visited);
    tensors.push_back(serialize_tensor(tensor, cmd_queue_ref));
}
...
static void serialize_graph(uint32_t device, const ggml_cgraph * cgraph, const std::shared_ptr<rpc_command_queue> & cmd_queue_ref, std::vector<uint8_t> & output) {
    ...
    for (uint32_t i = 0; i < n_nodes; i++) {
        add_tensor(cgraph->nodes[i], cmd_queue_ref, tensors, visited);
    }
