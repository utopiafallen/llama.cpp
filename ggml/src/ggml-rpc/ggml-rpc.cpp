...
static rpc_tensor serialize_tensor(const ggml_tensor * tensor, std::shared_ptr<rpc_dispatcher> const & dispatcher = nullptr) {
    rpc_tensor result;
    if (!tensor) {
...
static void add_tensor(ggml_tensor * tensor, std::shared_ptr<rpc_dispatcher> const & dispatcher, const ggml_cgraph * cgraph, std::vector<rpc_tensor> & tensors, std::unordered_set<ggml_tensor*> & visited) {
    if (tensor == nullptr) {
        return;
    }
    ...
    for (int i = 0; i < GGML_MAX_SRC; i++) {
        add_tensor(tensor->src[i], dispatcher, cgraph, tensors, visited);
    }
    add_tensor(tensor->view_src, dispatcher, cgraph, tensors, visited);
...
static uint8_t * serialize_graph(uint32_t device, const ggml_cgraph * cgraph, std::shared_ptr<rpc_dispatcher> const & dispatcher, size_t * output_size) {
    uint32_t n_nodes = cgraph->n_nodes;
    std::vector<rpc_tensor> tensors;
    std::unordered_set<ggml_tensor*> visited;
    for (uint32_t i = 0; i < n_nodes; i++) {
        add_tensor(cgraph->nodes[i], dispatcher, cgraph, tensors, visited);
    }
