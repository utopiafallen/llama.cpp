...
#if defined(GGML_HIP_USE_HIPCUB)
// hipCUB is the ROCm SDK's port of CUB with a matching Device*Sort API (namespace hipcub);
// it provides the large-ncols path (bitonic sort alone is limited by shared memory)
