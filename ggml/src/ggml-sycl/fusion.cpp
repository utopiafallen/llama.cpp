...
bool ggml_sycl_can_fuse(const ggml_cgraph * cgraph, int node_idx, std::initializer_list<enum ggml_op> ops,
    ...
    if (ops.size() == 2 && ops.begin()[0] == GGML_OP_UNARY && ops.begin()[1] == GGML_OP_MUL &&
        ...
        // The kernel derives one row stride per source from nb[1], so their rows must be
        // contiguous and equally shaped. The destination is written flat, which stays safe
        // under ggml-alloc's in-place reuse: reuse requires ggml_are_same_layout, and the MUL
        // output is freshly duplicated with packed nb, so the only operand it can be placed
        // on is one whose row stride already equals the row width.
        if (!ggml_is_contiguous_1(unary->src[0]) || !ggml_is_contiguous_1(other) ||
            !ggml_are_same_shape(other, unary) || !ggml_is_contiguous(mul)) {
            return false;
        }

        // past 2^31 elements the strided kernel's 32-bit fastdiv is inexact; decline rather
        // than abort, since the unfused path handles it
